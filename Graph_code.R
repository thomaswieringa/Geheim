...
for(cr in clickrates){
  ...
  for(fold in 1:folds){
    ...
    data <- data+read.csv(file = paste0('~/Documents/Seminar master/Rscript/Geheim/MAEscr',cr,'r20fold',fold,'.csv'))
    data2 <- data2 + read.csv(file = paste0('~/Documents/Seminar master/Rscript/Geheim/MAEsonTrainedcr',cr,'r20fold',fold,'.csv'))
    rank <- rank + read.csv(file = paste0('~/Documents/Seminar master/Rscript/Geheim/Rankscr',cr,'r20fold',fold,'.csv'))
  }
  data<-data/folds
  data2<-data2/folds
  rank <- rank/folds
...
fulldata <-fulldata/folds
fulldata2<-fulldata2/folds
fullrank <- fullrank/folds
...
plot(lambda, Rev(fulldata,margin = 1), type = "l", col = "black",  xlab = "Lambda", ylab = "MAE", ylim = range(0.0242,0.0267),xlim = range(0,exp(4)))
lines(lambda, Rev(fulldata2,margin=1), col = "red")
